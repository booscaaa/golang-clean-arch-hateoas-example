...
func TestDeleteItemUseCase(t *testing.T) {
	...
	mockItemRepository.EXPECT().Delete(1).Return(&fakeInsertItem, nil)

	itemUseCase := item.NewItemUseCase(mockItemRepository)

	item, err := itemUseCase.Delete(1)
...
func TestDeleteItemUseCase_Error(t *testing.T) {
	fakeInsertItem := domain.Item{}
	...
	defer mockCtrl.Finish()
	mockItemRepository := mocks.NewMockItemRepository(mockCtrl)
	mockItemRepository.EXPECT().Delete(1).Return(nil, fmt.Errorf("Any Error"))

	itemUseCase := item.NewItemUseCase(mockItemRepository)

	_, err = itemUseCase.Delete(1)
